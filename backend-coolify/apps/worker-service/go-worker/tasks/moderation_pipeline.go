...
func (deps *DependencyContext) ExecuteModerationPipeline(ctx context.Context, payload *PostModData) (ModerationReport, error) {
	...
	isExtractionMode := payload.ModerationTaskMode == ModerateAndExtractKeywords || payload.ModerationTaskMode == ExtractedKeywordsOnly
	...
	if isExtractionMode {
		...
		uniqueDetected := make(map[string]bool)
		for _, rawTopic := range finalResult.ExtractedTopics {
			trimmed := strings.TrimSpace(rawTopic)
			lowered := strings.ToLower(trimmed)

			if trimmed == "" || uniqueDetected[lowered] {
				continue
			}
			uniqueDetected[lowered] = true
			...
			if len(matchedDBTopics)+len(newlyDiscoveredTopics) == 3 {
				break
			}
		}
	}
	...
	if len(matchedDBTopics) > 0 || len(newlyDiscoveredTopics) > 0 {
		extractedTopicsPayload = append(matchedDBTopics, newlyDiscoveredTopics...)
	}

	determinedTopicSource := payload.TopicsSource
	if isExtractionMode && (determinedTopicSource == "" || len(newlyDiscoveredTopics) > 0 || len(matchedDBTopics) > 0) {
